<script lang="ts">
    ...
    async function loadMoreCourses() {
        ...
        const res = await fetch(
            `/api/courses/favorite?offset=${offset}&limit=${limit}`,
        );
    ...
    async function searchCourses(query: string, append = false) {
        ...
        const res = await fetch(
            `/api/courses/favorite?query=${encodeURIComponent(query)}&limit=${searchLimit}&offset=${searchOffset}`,
        );
...
<div class="max-w-7xl mx-auto space-y-8 px-4 lg:px-0">
    ...
    >
        <div>
            ...
            <p class="text-gray-600 mt-1">Courses you've saved as favorites</p>
        </div>
    </div>
    ...
    <div class="relative">
        ...
        {#if isSearching}
            <div
                class="absolute right-3 top-2.5 w-4 h-4 border-2 border-blue-300 border-t-blue-600 rounded-full animate-spin"
            ></div>
        {/if}
    </div>
    ...
    {#if courses.length === 0 && !loading}
        <div class="text-center py-16">
            ...
            <p class="text-gray-600 mb-6 max-w-sm mx-auto">
                Explore courses and tap the heart icon to favorite them.
            </p>
        </div>
    {:else}
            ...
            {#each courses as course}
                ...
                >
                    <div class="relative aspect-video overflow-hidden">
                        ...
                        />
                        <div
                            class="absolute inset-0 bg-opacity group-hover:bg-opacity-30 transition-all duration-300 flex items-center justify-center"
                        >
                            <div
                    ...
                    <div class="p-6">
                        ...
                        <div
                            class="flex justify-center items-center gap-3 mb-2"
                        >
                            <div class="flex-1">
                                {#if !course.isEnrolled}
                                    ...
                                    >
                                        {#if enrollLoading[course.id]}
                                            ...
                                            >
                                                <div
                                                    class="w-3 h-3 border-2 border-white border-t-blue-100 rounded-full animate-spin"
                                                ></div>
                                                Enrolling...
                                            </span>
                                        {:else}
                                ...
                                {:else}
                                    <div
                                        class="h-9 text-sm flex items-center justify-center bg-green-100 text-green-800 border border-green-300 rounded-md w-full"
                                    >
                                        Enrolled
                                    </div>
                                {/if}
                            ...
                            <button
                                ...
                                onclick={(e) => {
                                    e.stopPropagation();
                                    toggleFavorite(course.id, true);
                                }}
                                disabled={favoriteLoading[course.id]}
                            >
                                <Heart
                                    size={24}
                                    class="text-red-500 fill-red-500 stroke-none"
                                />
                            </button>
                        </div>
                        ...
                        >
                            ...
                            <Badge
                                variant="secondary"
                                class="text-xs px-3 py-1 bg-green-200 border-2 border-white"
                            >
                                Favorites
                            </Badge>
                        </div>
