<script lang="ts">
    ...
    async function loadMoreCourses() {
        ...
        loading = true;
        const res = await fetch(`/api/courses/enroll?offset=${offset}&limit=${limit}`);
    ...
    async function searchCourses(query: string, append = false) {
        ...
        const res = await fetch(`/api/courses/enroll?query=${encodeURIComponent(query)}&limit=${searchLimit}&offset=${searchOffset}`);
    ...
    async function toggleFavorite(courseId: string, current: boolean) {
        ...
        try {
            ...
            if (res.ok) {
                // ✅ Do NOT remove from the list — just update isFavorited
...
<div class="max-w-7xl mx-auto space-y-8 px-4 lg:px-0">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Your Learning Progress</h1>
            <p class="text-gray-600 mt-1">Courses you've enrolled in</p>
        </div>
    </div>
            ...
            {#each courses as course}
                ...
                >
                    <div class="relative aspect-video overflow-hidden">
                        <img
                            src={course.thumbnailUrl || "https://placehold.co/640x360?text=Thumbnail"}
                            alt={course.title}
